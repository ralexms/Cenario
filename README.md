...
Cenario is a local, privacy-focused meeting transcription and summarization tool designed to run entirely on your own hardware. It captures audio from both your microphone and system audio (for online meetings), transcribes it using OpenAI's Whisper, and generates summaries using local LLMs.
...
*   **Local Summarization**: Generates meeting summaries, action points, and Q&A using local LLMs (Qwen2.5, Llama 3.2, Phi-3.5). Supports chunked processing for long meetings and custom prompts.
*   **Web Interface**: A clean GUI with customizable themes (Light, Dark, Dracula, Nord, etc.) to manage recordings, view live previews, and run post-processing tasks.
...
*   **GPU**: NVIDIA GPU with CUDA recommended (4GB+ VRAM for summarization). Apple Silicon MPS is supported. CPU fallback is available on all platforms.
*   **Python**: 3.10+
...
5.  **Summarization**: Generate a summary and action points using the local LLM.
6.  **Export**: Download your files.
...
*   **Summarization Models**:
    ...
    *   `meta-llama/Llama-3.2-3B-Instruct`: Good quality (~1.8GB VRAM).
    *   `microsoft/Phi-3.5-mini-instruct`: Best quality (~2.2GB VRAM).
