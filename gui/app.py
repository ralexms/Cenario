...
def api_summarize_start():
    ...
    max_new_tokens = body.get('max_new_tokens')  # None = use detail-level default
    ...
    def run():
        ...
        try:
            ...
            # Summarize
            summary_text = summarizer.summarize(full_text, detail_level=detail_level, max_new_tokens=max_new_tokens, stream_callback=stream_cb, chunking=chunking, num_chunks=num_chunks, custom_prompt=custom_prompt)
...
def _ensure_ssl_cert():
    ...
    Tries three methods in order:
      1. system openssl command (no Python packages needed)
    ...
    # --- Method 3: pyopenssl adhoc (new cert each restart, browser re-warns) ---
    return 'adhoc'
